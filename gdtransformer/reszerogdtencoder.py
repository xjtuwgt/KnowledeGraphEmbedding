...
import torch
from gdtransformer.reszerographtransformer import gtransformerlayer
...
class graphTransformer(nn.Module):
    def __init__(self,
        ...
        self.trans_layers.append(gtransformerlayer(in_ent_feats=in_ent_dim, num_heads=num_heads, in_rel_feats=in_rel_dim, out_feats=hidden_dim, feat_drop=feat_drop, hop_num=self.hop_num,
        ...
        for l in range(1, num_layers):
            self.trans_layers.append(gtransformerlayer(in_ent_feats=hidden_dim,  num_heads=num_heads, in_rel_feats=hidden_dim, out_feats=hidden_dim, hop_num=self.hop_num,
    ...
    def forward(self, graph: DGLGraph, entity_embedder, rel_embedder, mask_edge_ids=None):
        ...
        for l in range(self.num_layers):
            h, h_r = self.trans_layers[l](graph, h, h_r, mask_edge_ids, drop_edges_ids)
