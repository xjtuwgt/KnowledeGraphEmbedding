...
def HypeParameterSpace(model_name):
    ...
    # smooth_factor = {'name': 'gamma', 'type': 'choice', 'values': [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]}
    ...
    # att_drop = {'name': 'att_drop', 'type': 'choice', 'values': [0.1, 0.25, 0.4, 0.5]}
    # top_k = {'name': 'top_k', 'type': 'choice', 'values': [4, 6, 8, 10]}
    ...
    # alpha = {'name': 'alpha', 'type': 'range', 'bounds': [0.1, 0.8]}
    ...
    rel_embed_dim = {'name': 'rel_embed_dim', 'type': 'fixed', 'value': 200}
    project_on = {'name': 'project_on', 'type': 'fixed', 'value': 1}
    negative_on = {'name': 'negative_on', 'type': 'fixed', 'value': 0}
    zero_on = {'name': 'zero_on', 'type': 'fixed', 'value': 1}
    loss_type = {'name': 'loss_type', 'type': 'fixed', 'value': 1}
    embed_dim = {'name': 'embed_dim', 'type': 'fixed', 'value': 512}
    ...
    adam_weight_decay = {'name': 'adam_weight_decay', 'type': 'range', 'bounds': [1e-5, 1e-8], "log_scale": True}
    conv_embed_shape1 = {'name': 'conv_embed_shape1', 'type': 'choice', 'values': [8, 16, 32]}
    conv_channels = {'name': 'conv_channels', 'type': 'fixed', 'value': 32}
    conv_filter_size = {'name': 'conv_filter_size', 'type': 'fixed', 'value': 3}
...
if __name__ == '__main__':
    generate_random_search_bash(model_name='ConvE', data_name='wn18rr', task_num=45)