...
def HypeParameterSpace(model_name):
    ...
    feat_drop = {'name': 'fea_drop', 'type': 'range', 'bounds': [0.2, 0.6]}
    input_drop = {'name': 'inp_drop', 'type': 'range', 'bounds': [0.2, 0.6]}
    ...
    zero_on = {'name': 'zero_on', 'type': 'fixed', 'value': 0}
...
def generate_random_search_bash(model_name, data_name, task_num, bash_save_path='../wn_jobs/'):
    ...
    for i in range(task_num):
        task_id, parameter_id = random_search_job.single_task_trial(i+2019)
...
if __name__ == '__main__':
    generate_random_search_bash(model_name='TuckER', data_name='wn18rr', task_num=30)