...
class KGERandomSearchJob(object):
    ...
    def single_task_trial(self, random_seed):
        ...
        """
        parameter_list = [[] for _ in range(37 + 1)]
        parameter_dict = {}
        ...
        parameter_dict['gamma'] = str(round(gamma, 4))
        ...
        zero_on = self.rand_search_parameter(self.search_space['zero_on'])
        parameter_dict['zo'] = str(zero_on)
        parameter_list[36] = zero_on

        parameter_list[37] = random_seed
        parameter_dict['s'] = str(random_seed)

        task_id = '_'.join([k + v for k, v in parameter_dict.items()])
